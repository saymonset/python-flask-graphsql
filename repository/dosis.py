...
def get_dosisWithoutFilter_repo():
    filter_query = {
        'status': {'$in': [True, 'True', 'true']}
    }

    return mongo.db.dosis.find(filter_query)
...
def delete_dosis_repo(id):
     ...
     return update_dosis_status(id);
     
def update_dosis_status(id):
    return mongo.db.dosis.update_one({"_id": ObjectId(id)}, {"$set": {"status": False}})

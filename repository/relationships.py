...
def delete_relationships_repo(id):
     ...
     return update_relationships_status(id)


def update_relationships_status(id):
    return mongo.db.relationships.update_one({"_id": ObjectId(id)}, {"$set": {"status": False}})  
...
def isValidBdRelationships(name):
    ...
    if campo:
        result = {
                'statusCode': 400,
                "resp":False,
                "message":"El nombre ya existe en bd",
            }
        return result

...
from repository.dosis import    get_dosis_repo, get_dosis_ByVaccine
...
def get_vaccdependet_service(dependentId):

    limite = int("100")
    desde = int("0")
    query = "";
    vaccines = get_vaccines_repo(limite, desde,  query)
    total = get_vaccines_counts_repo(query);
  
   
   # print(result)
    
    vaccine_aux = []  # Create a new list to store vaccine names
    
    #Obtenemos el dependiente
    data_dependent = get_dependentsbyId_servicWithOutJson(dependentId)

    for vaccine in vaccines:
        vaccineId = str(vaccine['_id']);
        #Obtenemos las dosis por cada vacuna
        dosis_ids = list(get_dosis_service_ByVaccine(vaccineId));
        ...
        #Numero de dias despues que nacio 
        days_birth = data_dependent['days_birth']
        #Un alert si  le faltra por vacunar
        isAlertApply = False
        for ad in appliedDosis:
            #Si la vacuna nop a sido aplicada y dias de haber nacido es mayor o igual a lo que expir la vacuna entonces
            if not ad['isApplied'] and int(days_birth) >= int(ad['expires_in_days']):
                isAlertApply = True

        vaccine['isAlertApply'] = isAlertApply
        vaccine_aux.append(vaccine)

    result = json_util.dumps(vaccine_aux)
    diccionario = {
        'total': total,
        'limite':limite,
        'desde':desde,
...
def get_vaccdependetAddDaysToBirthday_service(dependentId, addDaysToBirthday):
...
# Convertir addDaysToBirthday a entero
            daysNext = int(addDaysToBirthday) + int(days_birth) ;
            # print('addDaysToBirthday = '+ str(addDaysToBirthday))
            # print('days_birth = '+str(days_birth))
            # print('daysNext = '+str(daysNext))
            if not ad['isApplied'] and daysNext >= int(ad['expires_in_days']):
                isAlertApply = True
...
def vaccfindfromvaccidanddependetid_srv(vaccineId, dependentId):
    ...
    if dosis_ids is not None and  len(dosis_ids) > 0:
       ...
       #Obtenemos el dependiente
        data_dependent = get_dependentsbyId_servicWithOutJson(dependentId)
        vacc_vaccine_apply = []
        vacc_vaccine_apply.append({"dependent": data_dependent})
